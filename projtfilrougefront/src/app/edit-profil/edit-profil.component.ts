...
export class EditProfilComponent implements OnInit {
  ...
  formulaire: FormGroup = this.formBuilder.group({
    ...
    motDePasse: [this.authentification.utilisateur.prenom, [Validators.required]],
  });
  ...
  onSubmit() {
    console.log(this.idUser);
    const pt = this.formulaire.value;
    ...
    if (this.formulaire.valid) {
      if (this.idUser) {
        this.http
          ...
          .subscribe((resultat) => this.router.navigateByUrl('/profil'));
      } else {
        this.http
